...
/**
 * Cette attaque permet a l'élève d'attaquer tous les ennemis dans une zone en croix autour de l'eleve. Soit 2 cases vers le Nord, le Sud, l'Est et l'Ouest.
 *
 */
public class AttaqueCroix extends Attaque{

	public AttaqueCroix(Coor c) {
		super(c);
	}
	...
	public ArrayList<EnnemiModel> scanEnnemi(ObservableList<EnnemiModel> listeEnnemis) {
		...
		for (int i = 0; i < listeEnnemis.size(); i++) {
			if (this.estAPortee(listeEnnemis.get(i))) {
	 ...
	 * @return un boolean
	 */
	public boolean estAPortee(EnnemiModel e) {
		if (coor.estADeuxCasesEnLigne(e.getCoor()))
			return true;
		else
			return false;
	}
